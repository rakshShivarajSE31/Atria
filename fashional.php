...
if (!empty($usn) || !empty($name) || !empty($email) || !empty($phone) || !empty($sem) || !empty($date) || !empty($dancef) || !empty($teamn) || !empty($membersc) || !empty($message)) {
 $host = "localhost";
    ...
    if (mysqli_connect_error()) {
     ...
     $SELECT = "SELECT email From fashiontab Where email = ? Limit 1";
     $INSERT = "INSERT Into fashiontab (usn, name, email, phone, sem, date, dancef, teamn, membersc, message) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
     //Prepare statement
